...
const questionsData = [
  ...
  {
    id: 9,
    question: "Mỹ coi 'Ấp chiến lược' là 'xương sống' of 'Chiến tranh đặc biệt' nhằm mục đích chính là gì?",
    options: [
  ...
  if (isCorrect) {
    ...
    footerActions.innerHTML = `
      <button class="btn primary-btn large-btn" onclick="proceedToLuckyWithTransition()">
        <i class="fa-solid fa-gift"></i> 🎁 MỞ HỘP QUÀ
      </button>
    `;
  } else {
    // WRONG ANSWER LOGIC
...
// TRANSITION SCREEN WHEN ANSWERING CORRECTLY
function proceedToLuckyWithTransition() {
  ...
  setTimeout(() => {
    ...
    if (answeredQ === 9) {
      ...
      document.getElementById('luckySubtitleText').innerText = '🎉 Vẫn còn 3 Hộp Quà May Mắn đang chờ bạn khám phá!';
      showToast('🎉 CHÚC MỪNG! Bạn đã hoàn thành 9/9 câu hỏi!');
    } else {
...
function showQuestionSelectionView() {
  ...
  const answeredQ = questionsState.filter(q => q.status === 'answered_correct').length;
  if (answeredQ === 9) {
    document.getElementById('gameInfoBanner').innerText = '🎉 ĐÃ HOÀN THÀNH 9/9 CÂU HỎI! Vẫn còn 3 Hộp Quà May Mắn!';
  } else {
    document.getElementById('gameInfoBanner').innerText = 'Chọn 1 câu hỏi ➜ Trả lời ĐÚNG ➜ Mới được mở Hộp Quà May Mắn!';
  }
}

// DIRECT VIEW FOR REMAINING 3 GIFT BOXES
function openRemainingGiftsView() {
  ...
  const remainingCount = 12 - openedCount;

  if (answeredQ === 9) {
    document.getElementById('gameInfoBanner').innerText = `🎉 CHÚC MỪNG! BẠN ĐÃ HOÀN THÀNH 9/9 CÂU HỎI!`;
    document.getElementById('luckySubtitleText').innerText = `🎉 Chúc mừng bạn đã hoàn thành 9/9 câu hỏi! Hãy thoải mái chọn ${remainingCount} hộp quà còn lại bên dưới.`;
  }
}
...
function handleLuckyCardClick(id, cardEl, event) {
  ...
  if (!pendingLuckyUnlocked && answeredQ < 9) {
...
function openLuckyModal(lData) {
  ...
  if (claimBtn) {
    if (unopenedCount > 0) {
      claimBtn.innerHTML = `<i class="fa-solid fa-gift"></i> 🎁 TIẾP TỤC MỞ QUÀ`;
    } else {
      claimBtn.innerHTML = `<i class="fa-solid fa-trophy"></i> 🎉 HOÀN THÀNH TRÒ CHƠI`;
    }
  }
...
function claimLuckyReward() {
  ...
  if (answeredQ === 9) {
    if (unopenedCount > 0) {
      ...
      document.getElementById('finaleView').classList.remove('active');
      document.getElementById('luckyNumberView').classList.add('active');
      document.getElementById('luckySubtitleText').innerText = `🎉 Vẫn còn ${unopenedCount} Hộp Quà May Mắn đang chờ bạn khám phá!`;
      showToast(`Hãy chọn hộp quà tiếp theo để khám phá! 🎁 (${unopenedCount} hộp còn lại)`);
    } else {
      // ALL 12/12 BOXES OPENED! TRIGGER GRAND GAME SHOW FINALE SCREEN!
      showFinaleScreen();
    }
  } else {
    // Normal flow when < 9 questions answered correctly: return to Screen 1
    showQuestionSelectionView();
  }
}

// SHOW GAME SHOW FINALE VICTORY SCREEN
function showFinaleScreen() {
  ...
  showToast('🎉 CHÚC MỪNG! NHÓM 4 ĐÃ HOÀN THÀNH TRÒ CHƠI!');
}
...
function updateStats() {
  ...
  if (answeredQ === 9 && openedLucky < 12) {
    ...
    if (headerBtn) {
      ...
      headerBtn.innerHTML = `<i class="fa-solid fa-gift"></i> 🎁 XEM 3 HỘP QUÀ CÒN LẠI`;
    }
  } else {
